...
def mysql_to_excel(Client,file_name):
    ...
    db = pymysql.connect(user="jsms",password="JSMSpass",host="149.129.233.76",port=3307,database="mydepojavaPJn")
    sql1 = """
    SELECT * FROM 
    (SELECT 
    ...
    JOIN 
    (SELECT 
    COUNT(CASE WHEN CD.size = '20' AND EI.contCondition LIKE '%DMG%' THEN 1 END),
    COUNT(CASE WHEN CD.size = '40' AND EI.contCondition LIKE '%DMG%' THEN 1 END),
    COUNT(CASE WHEN CD.size = '45' AND EI.contCondition LIKE '%DMG%' THEN 1 END)
    FROM EIRIN EI 
    Left join ContainerDetails CD On CD.ContNo = EI.ContNo 
    WHERE CD.type = 'FR' 
    AND EI.PrincipleCode = '"""+Principle_Code+"""'
    ...
    JOIN 
    (SELECT 
    COUNT(CASE WHEN CD.size = '20' AND CS.ContCondition LIKE '%DMG%' THEN 1 END),
    COUNT(CASE WHEN CD.size = '40' AND CS.ContCondition LIKE '%DMG%' THEN 1 END),
    COUNT(CASE WHEN CD.size = '45' AND CS.ContCondition LIKE '%DMG%' THEN 1 END) 
    FROM ContainerStock CS 
    Left join ContainerDetails CD On CD.ContNo = CS.ContNo 
    Left Join EIRIN EI On EI.Nomor = CS.EIRNo 
    WHERE CD.type = 'FR' 
    ...
    COUNT(CASE WHEN CS.ContCondition LIKE '%DMG%' THEN 1 END) AS DMG,
    COUNT(CASE WHEN CS.ContCondition LIKE '%AV%' 
    OR CS.ContCondition LIKE '%DMG%' THEN 1 END) AS TOTAL 
    FROM ContainerStock CS 
    Left join ContainerDetails CD On CD.ContNo = CS.ContNo 
    Left Join EIRIN EI On EI.Nomor = CS.EIRNo 
    where CD.size = '20' AND CD.type = 'GP' 
    AND CS.PrincipleCode = '"""+Principle_Code+"""' 
    AND EI.DateIn >= (SELECT MIN(EIRIN.DateIn) FROM EIRIN) 
    AND EI.DateIn <= '"""+time_now+"""'  
    UNION ALL 
